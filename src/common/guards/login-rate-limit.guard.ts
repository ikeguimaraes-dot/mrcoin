...
/** 5 tentativas/min por IP e por conta (chaves Redis separadas), sliding window simples. */
@Injectable()
export class LoginRateLimitGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    await this.checkAndIncrement(`login-rl:ip:${ip}`);
    if (email) {
      await this.checkAndIncrement(`login-rl:account:${email}`);
    }
